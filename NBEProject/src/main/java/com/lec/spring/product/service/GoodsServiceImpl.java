...
public class GoodsServiceImpl implements GoodsService {
    ...
    public GoodsServiceImpl(SqlSession sqlSession){
        ...
        recentItemRepo = sqlSession.getMapper(RecentItemRepo.class);
...
//    최근 아이템 추가 기능 유저 아이디가 있어야 구현가능
    @Override
    public int addRecent(Integer userId, String goodsNo) {
        if(recentItemRepo.selectOne(userId, goodsNo) > 0){
            recentItemRepo.delete(userId, goodsNo);
        }
...
// 특정 사용자의 최근 목록 가져오기
    ...
    public List<Goods> getRecentItem(Integer userId) {
        return recentItemRepo.selectAll(userId).stream()
                .map(e -> {
                    return goodsRepo.selectById(e.getGoods_id());
                }).toList();
    }
}
